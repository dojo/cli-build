...
import HasPlugin, { StaticHasFeatures } from '../../../src/plugins/HasPlugin';
...
function runPluginTest(features: StaticHasFeatures, name: string, assertions: (args: CallbackArgs) => void) {
	return function () {
		...
		const plugin = new HasPlugin(features);
		...
		const addDependencyStub = stub(parser.state.current, 'addDependency');
...
registerSuite({
	...
	'no static flags': runPluginTest({}, 'has', ({ addDependencyStub, compiler, compilation, logStub }) => {
	...
	'static features': runPluginTest({ foo: true, bar: false }, 'has', ({ addDependencyStub, logStub }) => {
		assert.strictEqual(logStub.callCount, 3, 'should have logged to console three time');
	...
	'imports has - no default expressions': runPluginTest({ foo: true }, 'has-no-default', ({ addDependencyStub, logStub }) => {
	...
	'imports has - uses a variable for flag': runPluginTest({ foo: true }, 'has-call-var', ({ addDependencyStub, logStub }) => {
		assert.isFalse(logStub.called, 'should not have been called');
		assert.isFalse(addDependencyStub.called, 'should not have been called');
	}),

	'does not import has': runPluginTest({ foo: true }, 'has-no-import', ({ addDependencyStub, logStub }) => {
		assert.isFalse(logStub.called, 'should not have been called');
		assert.isFalse(addDependencyStub.called, 'should not have been called');
	}),
	...
	'no module compilation'() {
		...
		const { normalModuleFactory, parser } = compilation.params;
		normalModuleFactory.mockApply('parser', parser);
		parser.mockApply('program', getAst('has'));
	}
});
